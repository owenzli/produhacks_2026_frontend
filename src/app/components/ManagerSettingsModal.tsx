      ...
      {isOpen && form && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }} transition={{ duration: 0.2 }}
            className="relative bg-white rounded-3xl shadow-xl w-full max-w-2xl overflow-hidden max-h-[90vh] flex flex-col">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })}
                      className="w-full border border-gray-200 rounded-xl px-4 py-2.5 bg-gray-50 focus:bg-white text-gray-800 outline-none focus:ring-2 focus:ring-green-500/20 focus:border-green-500 transition-colors" />
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    ...
                    <select value={form.department} onChange={e => setForm({ ...form, department: e.target.value })}
                      className="w-full border border-gray-200 rounded-xl px-4 py-2.5 bg-gray-50 focus:bg-white text-gray-800 outline-none focus:ring-2 focus:ring-green-500/20 focus:border-green-500 transition-colors appearance-none">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  {form.privateDocs.map((doc, idx) => (
                    <div key={idx} className="flex items-center justify-between p-3 rounded-xl border border-gray-200 bg-white group hover:border-gray-300 transition-colors">
                ...
                <form onSubmit={addDoc} className="grid grid-cols-[1fr_1fr_auto] gap-3 mt-4">
                  <input type="text" placeholder="Title (e.g. Rubric...)" value={newTitle} onChange={e => setNewTitle(e.target.value)}
                    className="border border-gray-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-green-500/20 focus:border-green-500 outline-none" />
                  ...
                  <button type="submit" disabled={!newTitle || !newUrl}
                    className="flex justify-center items-center gap-1.5 bg-gray-100 border border-gray-200 hover:bg-gray-200 text-gray-700 font-medium px-4 py-2 rounded-xl text-sm transition-all disabled:opacity-50">
            ...
            <div className="px-6 py-4 flex items-center justify-end border-t border-gray-100 bg-gray-50/80 flex-shrink-0">
              ...
              <button onClick={handleSave} className="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-5 py-2.5 rounded-xl text-sm font-medium transition-all shadow-md shadow-green-200 active:scale-95">
